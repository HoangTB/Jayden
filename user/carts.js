    ...
    <div class="content-total">
      <h3>Total</h3>
       ...
       class="price">${productNumber} Sản phẩm</span></div>
        <div class=""><span class="text">Tổng số tiền:</span><span class="price">${cartCost.toLocaleString(
          "de-DE"
        )} VND </span></div>
         <button type="button" class="btn-cost" onclick="handleOrder()">Thanh Toán</button>
         </div>
`;
  ...
  if (cartItems && productCart) {
    Object.values(cartItems).map((item) => {
      productCartContent += `
      <div class="row">     
      <div><i class="fa-solid fa-trash fs-5 " onclick="deleteCarts('${
        item.id
      }')"></i></div>
